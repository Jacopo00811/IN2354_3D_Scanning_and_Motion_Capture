...
public:
	...
	double Eval(const Eigen::Vector3d& _x)
	{
		...
		Vector3d vec = _x - m_center;
		double Pippo = vec.squaredNorm() + m_radius * m_radius - m_a * m_a;
		return Pippo*Pippo - 4 * m_radius * m_radius * (vec.x() * vec.x() + vec.y() * vec.y());
	}
...
public:
	...
	double Eval(const Eigen::Vector3d& _x)
	{
		...
		for (int i = 0; i < m_coefficents.size() - 4; i++) {
			result += m_coefficents[i] * EvalBasis((m_funcSamp.m_pos[i] - _x).norm()); // Sum(alpha * ||phi - x ||^3)
		}

		Vector3d b = Vector3d(m_coefficents[m_coefficents.size() - 4], m_coefficents[m_coefficents.size() - 3], m_coefficents[m_coefficents.size() - 2]);
		result += b.dot(_x); // b dot product x 

		result += m_coefficents[m_coefficents.size() - 1]; // d
